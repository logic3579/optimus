...
func TestHTTPCredentialHTTPIntegration(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "bearer-token", string(consumed.Secret))
	...
	require.NotContains(t, string(consumeAudit.Payload), "bearer-token")
	...
	require.NoError(t, httpSvc.Delete(t.Context(), admin.ID, "127.0.0.1", "integration", bearerID))
